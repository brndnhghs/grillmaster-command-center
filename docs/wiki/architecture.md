...
- **Method Library** — 542 generators across `image_pipeline/methods/` (16 categories). Each file is self-contained and registers on import. See [`methods-library.md`](modules/methods-library.md).
...
- **Process-tree kill, cross-platform.** The Dashboard kills services via `_kill_tree()`: `os.killpg(os.getpgid(pid), signal.SIGTERM)` on POSIX, `taskkill /T /F` on Windows (no process groups there) — so any children the service spawned die too, preventing orphaned listeners after a stop. Port discovery is likewise dual-path: `lsof` on POSIX, `netstat -ano` on Windows.
- **GPU shader system with webgl2/gl330 parity.** The shader library (`core/shader_library/`) holds one GLSL source per shader; `build_fragment(name, "gl330"|"webgl2")` assembles it for the moderngl server path or the browser UI. VAO creation binds only attributes the compiler kept (`in_uv` is dead-code-eliminated when a fragment ignores `v_uv`). Colors travel BGR end-to-end (coerce_uniform pre-swaps, textures upload BGR, readback decodes BGR).
- **Watchdog observer is per-app-lifecycle.** `server.py` creates a fresh `Observer()` on every startup — on Windows the watchdog emitter thread blocks inside `ReadDirectoryChangesW` and a reused observer deadlocks startup.
