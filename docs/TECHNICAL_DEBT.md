...
|----|-------|----------|:---:|:---:|:---:|-----------------|--------|
| TD-01 | No end-to-end graph execution test (topo → propagation → terminal → frames) | Testing | High | Low | Low | `test_graph_executor_e2e.py` | ✅ Closed 2026-07-14 |
| TD-02 | No feedback-edge test (cycle support, frame-0 fallback) | Testing | High | Low | Low | `test_graph_feedback_edge.py` | ✅ Closed 2026-07-14 |
| TD-03 | No sim-cache eviction / per-node budget test | Testing/Perf | Med | Low | Low | Eviction test ✅ done; per-node cap (feature) | ✅ Both done 2026-07-14 |
| TD-04 | No param-keyframe edge-case test | Testing | Med | Low | Low | Add `_evaluate_param_track` tests | ✅ Closed 2026-07-14 |
| TD-05 | No group-node execution test | Testing | Med | Low | Low | `test_group_node_execution.py` | ✅ Closed 2026-07-14 |
| TD-06 | No graph save/load persistence test | Testing | Med | Low | Low | `test_graph_persistence.py` | ✅ Closed 2026-07-14 |
| TD-07 | `core/graph.py` holds 3D node defs (~130+ lines, client-side concern) | Architecture | Low | Med | Low | Extract to `core/threejs_nodes.py` | ✅ Closed 2026-07-14 |
...
| TD-15 | Keyframe easing contract footguns: (a) easing is read from the SEGMENT'S END keyframe (`kf_b`), so an easing set on the *start* keyframe is silently ignored; (b) an unknown/misspelled easing name (e.g. `"ease_in"` instead of `"ease-in"`) silently fell back to linear with no warning. STALE — core/easing.py now normalizes spelling (`_normalize_easing`) and warns+falls back to linear on unknown names (TD-15); end-keyframe convention documented in `apply_easing` docstring. Pinned by `test_easing_normalize.py`. Closed by concurrent work. | Quality | Low | Low | Low | Normalize + warn + document | Closed (stale) 2026-07-14 |
| TD-16 | `core/methods/simulations/flowfield.py:398` — `RuntimeWarning: overflow encountered in scalar add` when clamping glow color (`glow_col = tuple(min(255, c + 100) for c in col)`); `c` can exceed float range before `min`. Cosmetic now, but indicates unclamped HDR-ish color math. | Stability | Low | Low | Low | Clamp pre-add or use `np.clip` | ✅ Closed 2026-07-14 (this run) |
| TD-17 | `shootout/evaluator.py:115` — `Mean of empty slice` + `invalid value in divide` when `diffs` is empty (all-zero / static comparison). Produces NaN motion metric. | Stability | Med | Low | Low | Guard empty `diffs` before `.mean()` | ✅ Closed 2026-07-14; module removed 2026-07-21 |
...
| TD-15 | Keyframe easing normalization + warn | 2026-07-14 | this run |
| TD-16 | Flow-field glow color clip | 2026-07-14 | this run |
| TD-17 | Liveness NaN on <2-frame stacks | 2026-07-14 | this run |
| TD-19 | Langton palette symbol already imported | 2026-07-14 | ba45df7 |
