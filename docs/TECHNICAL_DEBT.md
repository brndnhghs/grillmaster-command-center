...
| TD-17 | `shootout/evaluator.py:115` — `Mean of empty slice` + `invalid value in divide` when `diffs` is empty (all-zero / static comparison). Produces NaN motion metric. | Stability | Med | Low | Low | Guard empty `diffs` before `.mean()` | ✅ Closed 2026-07-14 (this run) |
