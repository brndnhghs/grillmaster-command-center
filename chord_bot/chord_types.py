...
class HarmonicState:
    ...
    def copy(self) -> HarmonicState:
        # replace() shallow-copies scalar fields; we deepcopy the mutable lists
    ...
    def from_dict(cls, d: dict) -> HarmonicState:
        ...
        # Deepcopy mutable list fields so the caller's dict can't alias our state
        if "tensions" in kwargs:
            kwargs["tensions"] = list(kwargs["tensions"])
        if "voices" in kwargs:
            kwargs["voices"] = list(kwargs["voices"])
        if "tags" in kwargs:
