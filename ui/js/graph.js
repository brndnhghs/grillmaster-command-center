...
async function gToggleEditor3D() {
  ...
  if (window._gEditor3D && window._gEditor3D.isOpen()) {
    ...
    gSetStatus('3D editor closed');
    return;
  }
  const has3d = gNodes.some(n => ['__mesh3d__','__gltf__','__usd__','__light3d__','__camera3d__',
                                  '__scene_render__','__scene3d__'].includes(n.method_id));
  if (!has3d) { gShowToast('Add 3D nodes first (Mesh, Model, Light, Camera…)', true); return; }
  const ED = await import('/ui/js/editor3d.js');
  _gStopClientLive();
  if (gLiveMode) gToggleLive();          // stop server live stream if running
  gMainPreview.innerHTML = '';
  gMainPreview.classList.add('active');
  gPreviewShow();
  await ED.open({
    container: gMainPreview,
  ...
  gSetStatus('3D editor — click to select · W/E/R gizmo modes · F frame');
}
...
async function gSetLive(on) {
  ...
  if (on && await gGraphClientRenderable()) {
    gLiveMode = true;
    gLiveBtn.classList.toggle('active', true);
    gLiveBtnDesk.classList.toggle('active', true);
    try {
      ...
      if (gGraphHasClientNode()) {
        ...
        gLiveMode = false;
        gLiveBtn.classList.toggle('active', false);
        gLiveBtnDesk.classList.toggle('active', false);
        return;
      }
      gSetStatus('Client render unavailable — using server preview');
      // fall through to the server WS path below
    }
  }
  ...
  } catch(e) { gSetStatus('Live: ' + e.message); return; }
...
// ── Settings page ──────────────────────────────────────────────
// Theme values live in CSS (:root = slate, html[data-theme=…] overrides);
// this table only drives the picker cards.
const GM_THEMES = {
...
function gmApplyTheme(id) {
  if (!GM_THEMES[id]) id = 'slate';
  if (id === 'slate') delete document.documentElement.dataset.theme;
  else document.documentElement.dataset.theme = id;
  try { localStorage.setItem('gm-theme', id); } catch {}
...
(function () {
  ...
  const current = localStorage.getItem('gm-theme') || 'slate';
  for (const [id, t] of Object.entries(GM_THEMES)) {
    ...
    card.addEventListener('click', () => gmApplyTheme(id));
...
<div id="tl-ctx-menu">
  <button id="tl-ctx-mode" class="mode">↺ Switch to Loop</button>
  <button id="tl-ctx-split">✂ Split at playhead</button>
  <div class="sep"></div>
  <button id="tl-ctx-delete" class="danger">Delete clip</button>
</div>
...
<button id="server-restart-btn" title="Restart server (picks up code changes)">⟳</button>
<script>
(function () {
