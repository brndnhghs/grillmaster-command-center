...
## 2026-07-17 (cron-run) — FINISH leftover batch: bitangent-noise flow node 993
...
- ACTION: method 993 (Bitangent-Noise Particle Flow) was left uncommitted by a prior run with a HEADLINE MATH BUG — it claimed `v=(∂J/∂x,−∂J/∂y)` was divergence-free via `∇·v=∂²J/∂x∂y−∂²J/∂y∂x=0`, but that identity is wrong: the correct curl of the Jacobian determinant is `v=(∂J/∂y,−∂J/∂x)` for which mixed-partial commutation gives `∇·v=∂²J/∂y∂x−∂²J/∂x∂y=0`. Swapped the indices so the field is genuinely divergence-free (verified `mean|div|/mean|v| = 2.77e-17`, i.e. div-free by construction). Also added Rule-9 RGBA output (alpha = max-channel brightness, empty regions fully transparent) so it composites over upstream images, and rewrote the verification probe to use changed-pixel-fraction instead of a too-strict mean-Δ (sparse particle field on black); drift/evolve now show ~5% changed pixels. Registered in node-defs (522 defs; 993 present; is_time_varying=True). Verified headlessly: none Δ=0, drift/evolve animate, RGBA(512×768×4), server import clean (Rule 8).
- RECOMMENDATION: method 993 is a *divergence-free advection substrate* — a strong, cheap, genuinely time-varying target for shootout animation drivers (it survives the contrast-only liveness cull because particles actually move). Wire __lfo__/__noise1d__/__ramp__ into 993's scale/speed/anim_speed to prove the driver→pixel path works (the modulator dead-hotspots suggest it currently does not). Also fix the candidate-scan schema drift: genome `id`/`motifs` are None in the current dump, so the `seed_ids` promotion hook cannot be tied to stable genome IDs — log as a real gap (no fabrication).
