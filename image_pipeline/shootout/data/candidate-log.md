...
## 2026-07-14 (autonomous cg run — node 529 R2 Dither)
- CORPUS SCAN (real probe, 552 genomes): alive=193, dead/rejected=359 (65%), renders>150s=134 (24% timeout-cull), maxwall=547s. Stable/stagnant vs prior runs. DEAD hotspots remain driver/util attribution artifacts (__lfo__=917, __counter__=253, __noise1d__=144, __ramp__=116...); only ONE numbered method (137) appears. ALL 552 ids still None → survivor-seeding gap unchanged (safety-rule blocked).
- ACTION THIS RUN: researched + implemented **R2 Dither (529)** — Martin Roberts' 2018 R2 low-discrepancy 2D golden-ratio sequence as an ORDERED-DITHER threshold map (extremetechcentric.net/blog_posts/2018/12/24/...unreasonable-effectiveness-of-quasirandom-sequences), plus a temporal/STBN mode (Wolfe et al. 2021 "Scalar Spatiotemporal Blue Noise Masks", arxiv 2112.09629) that shifts the map per frame by an irrational step so frames are quasi-independent. Distinct from node 13 Dithering (Floyd-Steinberg/Bayer only — no R2, no temporal). 3 modes: r2 / temporal / compare_bayer. 8-step audit PASS: non-black std=0.480; none Δ=0.000000 (static); temporal-oscillate Δ=0.436; scroll Δ=0.483; oscillate Δ=0.061; contrast Δ=0.308; levels Δ=0.325; r2-vs-bayer Δ=0.350 (modes distinct). Registers in-process + /api/node-defs on :7871 (throwaway port, pitfall #12). Closed-form vectorized O(W*H) → never hits the timeout cull (cheap-alive class the shootout needs).
- WHY THIS HELPS THE SHOOTOUT: a sub-second banding-killing post/dither pass that takes a wired IMAGE and self-generates a gradient test; pairs with node 13 as an "ordered-dither family" and gives cheap-alive graphs a no-timeout-risk render-path improvement.
- RECOMMENDED NEXT: (a) GPU CLIENT_GPU_SHIMS twin of 529 (closed-form per-pixel R2 threshold → ideal additive client-GPU live path); (b) fix orphaned vhs.py id-527 collision (flagged in 967 run); (c) persist genome['id'] to enable survivor-seeding (longest-standing Route 8 #6 gap).
