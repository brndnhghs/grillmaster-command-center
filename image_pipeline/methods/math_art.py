...
def method_maze(out_dir: Path, seed: int, params=None):
    ...
    t = params.get("time", 0.0)
    seed_all(seed + int(t * 100))
    cs = params.get("cell_size", 10); algo = params.get("algorithm", "recursive_backtracker")
    geo = params.get("geometry", "rect"); style = params.get("style", "standard")
    pal_name = params.get("palette", ""); bg_pal_name = params.get("bg_palette", "")
    wall_thick = params.get("wall_thickness", 0.5); braid_p = params.get("braid", 0.0)
    loop_n = params.get("loops", 0); n_seeds = params.get("multi_seed", 1)
    show_sol = params.get("show_solution", "no"); ent_marks = params.get("entrance_marks", "no")
    grow_bias = params.get("growing_bias", 0.0); color_sat = params.get("color_saturation", 0.9)
    rings_c = params.get("rings", 0)
    from ..core.utils import PALETTES, quantize_to_palette, norm as norm_fn
    pal = PALETTES.get(pal_name, []); bg_pal = PALETTES.get(bg_pal_name, [])
    def pc(idx, pl=None): p = pl or pal; return p[idx % len(p)] if p else None
    if geo != "rect":
        img = np.ones((H,W,3),dtype=np.float32)*0.15; save(img.clip(0,1),mn(56,"Maze"),out_dir); return
    ...
    def _rb():
        ...
        while s:
            ...
            if nb:
                nr,nc,dr,dc = random.choice(nb); v[nr,nc]=True
    ...
    def _el():
        ...
        for r in range(rows-1):
            for c in range(cols-1):
                if random.random()<0.5 and f(c)!=f(c+1): vw[r,c+1]=False; u(c,c+1)
            for c in range(cols):
                if random.random()<0.4: hw[r+1,c]=False; sets[c]=c
        for c in range(cols-1):
    ...
    def _sw():
        for r in range(rows):
            rs = 0
            for c in range(cols):
                if r>0 and (c==cols-1 or random.random()<0.5):
                    cl = random.randint(rs,c); hw[r,cl]=False; rs=c+1
                elif r>0: vw[r,c]=False
    def _gt():
        v = np.zeros((rows,cols),dtype=bool); cl = [(0,0)]; v[0,0]=True
        while cl:
            if grow_bias>=0: idx = -1 if random.random()<grow_bias else random.randint(0,len(cl)-1)
            else: idx = 0 if random.random()<-grow_bias else random.randint(0,len(cl)-1)
            r,c = cl[idx]; nb = []
            for dr,dc in [(-1,0),(1,0),(0,-1),(0,1)]:
                nr,nc = r+dr,c+dc
                if 0<=nr<rows and 0<=nc<cols and not v[nr,nc]: nb.append((nr,nc,dr,dc))
...
def method_lowpoly(out_dir: Path, seed: int, params=None):
    ...
    seed_all(seed + int(t * 100))
    n_pts = params.get("points",200); jitter = params.get("jitter",10)
    dist = params.get("point_distribution","uniform"); mesh = params.get("mesh_type","delaunay")
    color_src = params.get("color_source","position"); pal_name = params.get("palette","")
    style = params.get("style","filled"); bg_style = params.get("bg_style","dark")
    edge_col = params.get("edge_color","auto"); edge_w = params.get("edge_width",1.0)
    light_ang = params.get("light_angle",45); light_alt = params.get("light_altitude",30)
    extrude = params.get("extrusion_scale",0.5); grad_blend = params.get("gradient_blend",0.5)
    noise_amp = params.get("noise_amplitude",0.0); adaptive = params.get("adaptive_detail","no")
    from ..core.utils import PALETTES, quantize_to_palette
    pal = PALETTES.get(pal_name,[])
    pts = []
    if dist=="uniform": pts = [(random.uniform(0,W),random.uniform(0,H)) for _ in range(n_pts)]
    elif dist=="grid_jitter":
        cols = int(math.sqrt(n_pts*W/H)); rows = n_pts//cols
        for r in range(rows):
            for c in range(cols): pts.append(((c+0.5)*W/cols+random.uniform(-jitter,jitter),(r+0.5)*H/rows+random.uniform(-jitter,jitter)))
    elif dist=="fibonacci":
        for i in range(n_pts):
            theta = i*math.pi*(3-math.sqrt(5)); r = math.sqrt(i/n_pts)*min(W,H)*0.45
        ...
        edges = np.abs(noise)>0.5; cand = list(zip(*np.where(edges)))
        if len(cand)<n_pts: cand = [(random.randint(0,H-1),random.randint(0,W-1)) for _ in range(n_pts)]
        sel = random.sample(cand,min(n_pts,len(cand)))
        pts = [(y,x) for x,y in sel]
        if adaptive=="yes" and dist=="multi_res": pts += [(random.uniform(0,W),random.uniform(0,H)) for _ in range(n_pts//3)]
        ...
        for _ in range(n_pts):
            while True:
                x,y = random.uniform(0,W-1),random.uniform(0,H-1)
                if random.random()<density[int(y),int(x)]: pts.append((x,y)); break
    pts.extend([(0,0),(W-1,0),(0,H-1),(W-1,H-1)])
    pts = np.array(pts,dtype=np.float32)
    if noise_amp>0: pts += np.random.randn(*pts.shape)*noise_amp
    from scipy.spatial import Delaunay, Voronoi
    tri = Delaunay(pts)
    img = np.zeros((H,W,3),dtype=np.float32)
    if bg_style=="dark": img[:,:,:]=0.05
    elif bg_style=="light": img[:,:,:]=0.95
    elif bg_style=="gradient":
        yy,xx = np.ogrid[:H,:W]; g = (xx/W+yy/H)*0.5
        img[:,:,0]=g*0.1; img[:,:,1]=g*0.08; img[:,:,2]=g*0.15
    def gc(cen,idx):
        cx,cy = cen
        if color_src=="position": return np.array([cx/W,cy/H,0.5+0.5*math.sin(t+cx*0.01)],dtype=np.float32)
        if color_src=="palette" and pal: c = pal[idx%len(pal)]; return np.array(c,dtype=np.float32)/255.0
        if color_src=="gradient": v = (cx/W+cy/H)*0.5; return np.array([v,0.3,1.0-v],dtype=np.float32)
        if color_src=="random_palette" and pal: c = random.choice(pal); return np.array(c,dtype=np.float32)/255.0
        if color_src=="noise": n = math.sin(cx*0.05+t)*math.cos(cy*0.05+t*0.7); return np.array([n*0.5+0.5,0.3,0.5-n*0.3],dtype=np.float32)
        if color_src=="brightness": v = (cx/W+cy/H)*0.5; return np.array([v,v,v],dtype=np.float32)
    ...
    if mesh in ("delaunay","delaunay_wireframe","filled_wireframe","glow_edges","dual_layer","shaded_3d","gradient_fill","noise_displaced"):
        for i,simplex in enumerate(tri.simplices):
            p3 = pts[simplex]; cen = p3.mean(axis=0); col = gc(cen,i)
            if style=="shaded_3d":
                v1 = p3[1]-p3[0]; v2 = p3[2]-p3[0]
                n = np.cross(np.append(v1,extrude),np.append(v2,extrude))
                nl = np.linalg.norm(n)
                if nl>0:
                    n = n/nl; ld = np.array([math.cos(light_ang*math.pi/180),math.sin(light_ang*math.pi/180),math.sin(light_alt*math.pi/180)])
                    ld = ld/np.linalg.norm(ld); shade = max(0.3,np.dot(n,ld)); col = col*shade
            if style=="gradient_fill": col = col*(1-grad_blend)+col*1.3*grad_blend
            if style=="noise_displaced": p3 = p3+np.random.randn(3,2)*noise_amp
            ...
            if style=="glow_edges":
                for j in range(3): cv2.line(img,tuple(pi[j]),tuple(pi[(j+1)%3]),(0.8,0.6,0.2),int(edge_w+2))
            if style=="dual_layer":
                cen2 = pi.mean(axis=0).astype(np.int32); ip = ((pi-cen2)*0.8+cen2).astype(np.int32)
    ...
    if mesh=="voronoi":
        ...
        for i,ri in enumerate(vor.point_region):
            ...
            cen = pts[i] if i<len(pts) else poly.mean(axis=0); col = gc(cen,i)
    ...
    capture_frame('73', img); save(img.clip(0,1), mn(73,"Low Poly"), out_dir)
...
@method(id="76", name="Binary Counter", category="math_art", tags=["code","fast", "expanded"],
         params={"bits":{"description":"bit rows","min":4,"max":16,"default":8},
                 ...
                 "data_source":{"description":"data source","choices":["x_position","input_image","sine_wave","noise","prime_sequence","time_animated","gray_code","fibonacci","audio_waveform"],"default":"x_position"},
                 "layout":{"description":"layout","choices":["vertical_rows","horizontal_cols","radial_rings","spiral","matrix_rain","barcode","3d_perspective"],"default":"vertical_rows"},
                 "style":{"description":"style","choices":["solid_bars","palette","gradient_bars","glow_bars","led_matrix","oscilloscope","heat_map","rgb_per_bit","morse_overlay"],"default":"solid_bars"},
                 "palette":{"description":"PALETTES","default":""},
                 ...
                 "time":{"description":"time","min":0.0,"max":100.0,"default":0.0}})
def method_binary_counter(out_dir: Path, seed: int, params=None):
    import cv2
    if params is None: params = {}
    t = float(params.get("time", 0.0))
    seed_all(seed + int(t * 100))
    bits = params.get("bits",8); glitch_n = params.get("glitch_lines",50)
    data_src = params.get("data_source","x_position"); layout = params.get("layout","vertical_rows")
    style = params.get("style","solid_bars"); pal_name = params.get("palette","")
    glitch = params.get("glitch_type","none"); glitch_int = params.get("glitch_intensity",0.3)
    bg_style = params.get("bg_style","dark"); spacing = params.get("bit_spacing",4)
    bar_h = params.get("bar_height",0.8); glow_r = params.get("glow_radius",3)
    led_r = params.get("led_radius",3); scan_speed = params.get("scanline_speed",1.0)
    from ..core.utils import PALETTES, quantize_to_palette
    pal = PALETTES.get(pal_name,[])
    img = np.zeros((H,W,3),dtype=np.float32)
    if bg_style=="dark": img[:,:,:]=0.05
    elif bg_style=="light": img[:,:,:]=0.9
    elif bg_style=="gradient":
        yy,xx = np.ogrid[:H,:W]; img[:,:,0]=yy/H*0.1; img[:,:,1]=xx/W*0.08; img[:,:,2]=0.05
    elif bg_style=="grid":
        img[:,:,:]=0.05
        for x in range(0,W,20): cv2.line(img,(x,0),(x,H),(0.1,0.1,0.1),1)
        for y in range(0,H,20): cv2.line(img,(0,y),(W,y),(0.1,0.1,0.1),1)
    elif bg_style=="scanline_bg":
        img[:,:,:]=0.05
        for y in range(0,H,3): img[y:y+1,:,:]=0.08
    data = np.arange(W)
    if data_src=="sine_wave": data = ((np.sin(np.arange(W)*0.1+t)+1)*((1<<bits)//2)).astype(np.int32)
    elif data_src=="noise": data = np.random.randint(0,1<<bits,W)
    elif data_src=="prime_sequence":
        n = W+100; ip = np.ones(n,dtype=bool); ip[:2]=False
        for p in range(2,int(n**0.5)+1):
            if ip[p]: ip[p*p:n:p]=False
        data = (np.where(ip)[0][:W]%(1<<bits)).astype(np.int32)
    elif data_src=="time_animated": data = (np.sin(np.arange(W)*0.05+t)*np.cos(np.arange(W)*0.03+t*0.7)*(1<<bits)//2).astype(np.int32)
    elif data_src=="gray_code": raw = np.arange(W); data = (raw^(raw>>1))%(1<<bits)
    elif data_src=="fibonacci":
        a,b=0,1; fib=[]
        for _ in range(W): fib.append(a); a,b = b,(a+b)%(1<<bits)
    ...
    if glitch in ("bit_flip","all") and glitch_int>0:
        fm = np.random.random(data.shape)<glitch_int*0.1
        data[fm] ^= 1<<np.random.randint(0,bits,fm.sum())
    ba = np.zeros((bits,W),dtype=bool)
    for b in range(bits): ba[b] = (data>>b)&1
    if layout=="vertical_rows":
        row_h = H//(bits+1)
        for b in range(bits):
            y0 = b*(row_h+spacing)+spacing; y1 = y0+int(row_h*bar_h)
            for x in range(W):
                if ba[b,x]:
                    col = [0.8,0.6,0.1]
                    if style=="palette" and pal: col = [c/255.0 for c in pal[b%len(pal)]]
                    elif style=="gradient_bars": col = [b/bits,0.3,1-b/bits]
                    elif style=="heat_map": col = [b/bits,0.2,1-b/bits]
                    elif style=="rgb_per_bit": col = [b%3==0 and 1 or 0,b%3==1 and 1 or 0,b%3==2 and 1 or 0]
                    elif style=="oscilloscope": col = [0.2,0.8,0.2]
                    img[y0:y1,x:x+1] = col
                    if style=="glow_bars": cv2.GaussianBlur(img[max(0,y0-glow_r):min(H,y1+glow_r),max(0,x-glow_r):min(W,x+glow_r+1)],(0,0),sigmaX=glow_r,dst=img[max(0,y0-glow_r):min(H,y1+glow_r),max(0,x-glow_r):min(W,x+glow_r+1)])
                    if style=="led_matrix": cv2.circle(img,(x,(y0+y1)//2),led_r,col,-1)
    elif layout=="horizontal_cols":
        col_w = W//(bits+1)
        for b in range(bits):
            x0 = b*(col_w+spacing)+spacing; x1 = x0+col_w
            for y in range(H):
                if ba[b,y*W//H%W]: img[y,x0:x1]=[0.8,0.6,0.1]
    elif layout=="radial_rings":
        cx,cy = W//2,H//2
        for b in range(bits):
            r = (b+1)*min(W,H)//(bits*2)
            for a in range(360):
                th = a*math.pi/180; x = int(cx+r*math.cos(th)); y = int(cy+r*math.sin(th))
                if 0<=x<W and 0<=y<H and ba[b,a*W//360%W]: img[y,x]=[0.8,0.6,0.1]
    elif layout=="matrix_rain":
        for x in range(W):
            for b in range(bits):
                if ba[b,x]:
                    y = int((b/bits)*H+t*scan_speed*10)%H; img[y,x]=[0.1,0.8,0.1]
                    for tr in range(1,6): img[(y-tr)%H,x]=[0.05,0.4-tr*0.06,0.05]
    elif layout=="barcode":
        for x in range(W):
            v = data[x] if x<len(data) else 0; col = [v/(1<<bits),0.5,1-v/(1<<bits)]
            cv2.line(img,(x,0),(x,H),col,1)
    elif layout=="3d_perspective":
        for b in range(bits):
            sc = 1.0-b*0.08; y0 = int(H*(1-sc)/2); y1 = int(H-y0)
            for x in range(W):
                if ba[b,x]: img[y0:y1,x] = [0.8*sc,0.6*sc,0.1*sc]
    if glitch in ("random_lines","all"):
        for _ in range(int(glitch_n*glitch_int)):
            y = random.randint(0,H-1); x = random.randint(0,W-20); w = random.randint(5,30)
            img[y,x:x+w] = [random.random(),random.random(),random.random()]
    if glitch in ("row_shift","all"):
        for _ in range(int(glitch_n*glitch_int*0.3)):
            y = random.randint(0,H-1); img[y] = np.roll(img[y],random.randint(-20,20),axis=0)
    if glitch in ("color_noise","all"): img = (img+np.random.randn(H,W,3)*glitch_int*0.3).clip(0,1)
    if glitch in ("scanline","all"):
        ph = int(t*scan_speed*10)%H
        for y in range(ph,H,4): img[y:y+1,:]*=0.5
    if glitch in ("vhs_tracking","all"):
        bh = random.randint(3,10); y = random.randint(0,H-bh); sh = int(math.sin(t*3+y*0.1)*10)
        img[y:y+bh] = np.roll(img[y:y+bh],sh,axis=1)
    if glitch in ("pixel_sort","all"):
        for _ in range(3):
            y = random.randint(0,H-1); x0 = random.randint(0,W-50); x1 = min(W,x0+random.randint(20,60))
            seg = img[y,x0:x1].copy(); img[y,x0:x1] = seg[np.argsort(seg.sum(axis=1))]
    if pal_name and pal_name in PALETTES: img = quantize_to_palette(img.clip(0,1),pal_name)
    capture_frame('76', img); save(img.clip(0,1), mn(76,"Binary Counter"), out_dir)
...
@method(id="78", name="Circle Packing", category="math_art", tags=["packing","fast", "expanded"],
         params={"max_circles":{"description":"max circles","min":50,"max":500,"default":200},
                 ...
                 "packing":{"description":"strategy","choices":["random","radial","spiral","hex_grid","input_weighted","relaxation","fibonacci","bouncing"],"default":"random"},
                 "color_source":{"description":"coloring","choices":["random","palette","input_image","radius","position","overlap_gradient","density","gradient_fill","depth"],"default":"random"},
                 "palette":{"description":"PALETTES","default":""},
                 "style":{"description":"style","choices":["filled","wireframe","concentric","sunburst","halftone","voronoi_fill","shadow","mosaic"],"default":"filled"},
                 "bg_style":{"description":"bg","choices":["dark","light","gradient","grid","input_image"],"default":"dark"},
                 ...
                 "pulse_amplitude":{"description":"pulse","min":0.0,"max":0.5,"default":0.0},
                 "drift_speed":{"description":"drift","min":0.0,"max":5.0,"default":0.0},
                 "time":{"description":"time","min":0.0,"max":100.0,"default":0.0}})
def method_circle_packing(out_dir: Path, seed: int, params=None):
    ...
    t = float(params.get("time", 0.0))
    seed_all(seed + int(t * 100))
    max_c = params.get("max_circles",200); r_min = params.get("min_radius",4); r_max = params.get("max_radius",25)
    attempts = params.get("attempts",10000); pack = params.get("packing","random")
    color_src = params.get("color_source","random"); pal_name = params.get("palette","")
    style = params.get("style","filled"); bg_style = params.get("bg_style","dark")
    outline_w = params.get("outline_width",1); outline_c = params.get("outline_color","auto")
    gap = params.get("gap",1); conc_r = params.get("concentric_rings",3); sun_r = params.get("sunburst_rays",8)
    half_d = params.get("halftone_density",0.5); relax_i = params.get("relaxation_iters",5)
    pulse_a = params.get("pulse_amplitude",0.0); drift_s = params.get("drift_speed",0.0)
    from ..core.utils import PALETTES, quantize_to_palette
    pal = PALETTES.get(pal_name,[])
    img = np.zeros((H,W,3),dtype=np.float32)
    if bg_style=="dark": img[:,:,:]=0.05
    ...
    def can_place(x,y,r):
        for cx,cy,cr in circles:
            if math.hypot(x-cx,y-cy)<r+cr+gap: return False
        return True
    def gc(idx,x,y,r):
        if color_src=="random": return [random.random(),random.random(),random.random()]
        if color_src=="palette" and pal: return [c/255.0 for c in pal[idx%len(pal)]]
        if color_src=="radius": return [r/r_max,0.3,1-r/r_max]
        if color_src=="position": return [x/W,y/H,0.5]
        if color_src=="gradient_fill": return [(x+y)/(W+H),0.3,1-(x+y)/(W+H)]
        if color_src=="depth": return [r/r_max*0.8+0.2,r/r_max*0.5+0.1,0.3]
        if color_src=="density": return [len(circles)/max_c,0.3,1-len(circles)/max_c]
        return [0.5,0.3,0.5]
    if pack=="random":
        for _ in range(attempts):
            if len(circles)>=max_c: break
            x,y = random.uniform(r_max,W-r_max),random.uniform(r_max,H-r_max)
            r = random.uniform(r_min,r_max)
            if can_place(x,y,r): circles.append((x,y,r))
    elif pack=="radial":
        cx,cy = W/2,H/2
        for _ in range(attempts):
            if len(circles)>=max_c: break
            a = random.uniform(0,2*math.pi); d = random.uniform(0,min(W,H)*0.4)
            x,y = cx+d*math.cos(a),cy+d*math.sin(a); r = random.uniform(r_min,min(r_max,d*0.5+5))
            if can_place(x,y,r): circles.append((x,y,r))
    elif pack=="spiral":
        cx,cy = W/2,H/2
        for i in range(attempts):
            if len(circles)>=max_c: break
            th = i*0.5+t; rd = i*0.5; x = cx+rd*math.cos(th); y = cy+rd*math.sin(th)
            if 0<=x<W and 0<=y<H:
                r = random.uniform(r_min,min(r_max,rd*0.2+3))
                if can_place(x,y,r): circles.append((x,y,r))
    elif pack=="hex_grid":
        sp = r_max*2+gap
        for row in range(int(H/(sp*0.866))+1):
            for col in range(int(W/sp)+1):
                if len(circles)>=max_c: break
                x = col*sp+(row%2)*sp*0.5; y = row*sp*0.866; r = random.uniform(r_min,r_max)
                if can_place(x,y,r): circles.append((x,y,r))
    elif pack=="fibonacci":
        for i in range(max_c):
            th = i*math.pi*(3-math.sqrt(5)); rd = math.sqrt(i/max_c)*min(W,H)*0.45
            x = W/2+rd*math.cos(th+t); y = H/2+rd*math.sin(th+t)
            r = random.uniform(r_min,max(r_min,r_max*(1-i/max_c*0.5)))
            if can_place(x,y,r): circles.append((x,y,r))
    elif pack in ("input_weighted","relaxation","bouncing"):
        for _ in range(attempts):
            if len(circles)>=max_c: break
            x,y = random.uniform(r_max,W-r_max),random.uniform(r_max,H-r_max)
            r = random.uniform(r_min,r_max)
    ...
    for idx,(x,y,r) in enumerate(circles):
        if pulse_a>0: r *= (1+pulse_a*math.sin(t*2+idx*0.3))
        if drift_s>0: x += math.sin(t*drift_s+idx*1.7)*5; y += math.cos(t*drift_s+idx*2.3)*5
        col = gc(idx,x,y,r); xi,yi,ri = int(x),int(y),int(max(1,r))
        if style=="filled":
        ...
        elif style=="wireframe": cv2.circle(img,(xi,yi),ri,col,1)
        elif style=="concentric":
            for cr in range(conc_r,0,-1): cv2.circle(img,(xi,yi),int(ri*cr/conc_r),[c*(0.5+0.5*cr/conc_r) for c in col],-1)
        elif style=="sunburst":
            cv2.circle(img,(xi,yi),ri,col,-1)
            for a in range(sun_r):
                ang = a*2*math.pi/sun_r+t; ex = int(x+ri*0.8*math.cos(ang)); ey = int(y+ri*0.8*math.sin(ang))
                cv2.line(img,(xi,yi),(ex,ey),(1,1,1),1)
        elif style=="halftone":
            for _ in range(int(half_d*20)):
                hx = int(x+random.uniform(-ri,ri)); hy = int(y+random.uniform(-ri,ri))
                if math.hypot(hx-xi,hy-yi)<ri: cv2.circle(img,(hx,hy),1,col,-1)
        elif style=="shadow":
            cv2.circle(img,(xi+3,yi+3),ri,(0,0,0),-1); cv2.circle(img,(xi,yi),ri,col,-1)
        elif style=="mosaic":
            sz = max(2,ri//4)
            for dy in range(-ri,ri,sz):
                for dx in range(-ri,ri,sz):
                    if dx*dx+dy*dy<ri*ri: img[yi+dy:yi+dy+sz,xi+dx:xi+dx+sz] = col
    if pal_name and pal_name in PALETTES: img = quantize_to_palette(img.clip(0,1),pal_name)
    capture_frame('78', img); save(img.clip(0,1), mn(78,"Circle Packing"), out_dir)

@method(id="62", name="Chaotic Map", category="math_art", tags=["chaos","slow", "expanded"],
         params={"map_type":{"description":"map type","choices":["henon","logistic","tinkerbell","gingerbreadman","ikeda","lorenz","standard_map","bakers_map","arnold_cat","duffing","rossler","multi_blend"],"default":"henon"},
                 ...
                 "palette":{"description":"PALETTES","default":""},
                 "color_mode":{"description":"coloring","choices":["density","iteration","gradient","multi_map","velocity","divergence"],"default":"density"},
                 "bg_style":{"description":"bg","choices":["dark","glow","gradient","paper"],"default":"dark"},
                 "multi_maps":{"description":"multi maps","default":"henon,ikeda"},"poincare_mod":{"description":"poincare mod","min":2,"max":50,"default":10},
...
def method_chaotic_map(out_dir: Path, seed: int, params=None):
    ...
    t = float(params.get("time", 0.0))
    seed_all(seed + int(t * 100))
    mt = params.get("map_type","henon"); a = params.get("a",1.4); b = params.get("b",0.3)
    c = params.get("c",2.0); d = params.get("d",0.5); n = params.get("n",500000)
    di = params.get("density_inc",0.002); style = params.get("style","density")
    pal_name = params.get("palette",""); cm = params.get("color_mode","density")
    bg = params.get("bg_style","dark"); multi = params.get("multi_maps","henon,ikeda")
    poinc = params.get("poincare_mod",10); bifp = params.get("bifurcation_param","a")
    bifmin = params.get("bifurcation_min",1.0); bifmax = params.get("bifurcation_max",1.8)
    trace_len = params.get("trace_length",100); ls = params.get("lorenz_sigma",10)
    lr = params.get("lorenz_rho",28); lb = params.get("lorenz_beta",2.667)
    ...
    img = np.zeros((H,W,3),dtype=np.float32)
    if bg=="dark": img[:,:,:]=0.02
    elif bg=="glow": img[:,:,:]=0.01
    elif bg=="gradient":
        yy,xx = np.ogrid[:H,:W]; img[:,:,0]=yy/H*0.08; img[:,:,1]=xx/W*0.06; img[:,:,2]=0.1
    elif bg=="paper": img[:,:,:]=0.92
    def ms(x,y,z,mt,a,b,c,d,t):
        if mt=="henon": return (1-a*x*x+y,b*x,z)
        elif mt=="logistic": return (a*x*(1-x),y,z)
        elif mt=="tinkerbell": return (x*x-y*y+a*x+b*y,2*x*y+c*x+d*y,z)
        elif mt=="gingerbreadman": return (1-y+abs(x),x,z)
        elif mt=="ikeda":
            u = 0.4-6.0/(1+x*x+y*y); return (1+c*(x*math.cos(u)-y*math.sin(u)),c*(x*math.sin(u)+y*math.cos(u)),z)
        elif mt=="lorenz":
            dt=0.01; dx=ls*(y-x)*dt; dy=(x*(lr-z)-y)*dt; dz=(x*y-lb*z)*dt; return (x+dx,y+dy,z+dz)
        elif mt=="standard_map":
            yn = (y+a*math.sin(x))%(2*math.pi); return ((x+yn)%(2*math.pi),yn,z)
        elif mt=="bakers_map":
            if x<0.5: return (2*x,a*y,z)
            else: return (2*x-1,b*y+0.5,z)
        elif mt=="arnold_cat": return ((x+y)%1,(x+2*y)%1,z)
        elif mt=="duffing":
            dt=0.01; dx=y*dt; dy=(-a*y-x*x*x+b*math.cos(t))*dt; return (x+dx,y+dy,z)
        elif mt=="rossler":
            dt=0.01; dx=(-y-z)*dt; dy=(x+a*y)*dt; dz=(b+z*(x-c))*dt; return (x+dx,y+dy,z+dz)
        return (x,y,z)
    def ts(x,y,z):
        if mt=="lorenz":
            if lproj=="xy": sx,sy=x,y
            elif lproj=="xz": sx,sy=x,z
            elif lproj=="yz": sx,sy=y,z
            else: ang=t*0.5; sx=x*math.cos(ang)-y*math.sin(ang); sy=x*math.sin(ang)+y*math.cos(ang)
        else: sx,sy=x,y
        return int((sx+2)/4*W),int((sy+2)/4*H)
    if style=="bifurcation":
        vals = np.linspace(bifmin,bifmax,W)
        for px,val in enumerate(vals):
            x=y=z=0.5; p={"a":a,"b":b,"c":c,"d":d}; p[bifp]=val
            for _ in range(500): x,y,z = ms(x,y,z,mt,p["a"],p["b"],p["c"],p["d"],t)
            for _ in range(200):
                x,y,z = ms(x,y,z,mt,p["a"],p["b"],p["c"],p["d"],t)
                py = int((x+2)/4*H)
                if 0<=py<H: density[py,px]+=di
    else:
        x=y=z=0.5
        for _ in range(1000): x,y,z = ms(x,y,z,mt,a,b,c,d,t)
        trail = []
        for i in range(n):
            x,y,z = ms(x,y,z,mt,a,b,c,d,t+i*0.0001)
            if style=="poincare" and i%poinc!=0: continue
            px,py = ts(x,y,z)
            if 0<=px<W and 0<=py<H:
                if style=="density": density[py,px]+=di
                elif style=="trace":
                    trail.append((px,py))
                    if len(trail)>trace_len: trail.pop(0)
                elif style=="orbit_trail":
                    trail.append((px,py))
                    if len(trail)>trace_len: trail.pop(0)
                    for j,(tx,ty) in enumerate(trail):
                        a2 = j/len(trail); img[ty,tx]=[a2,0.3,1-a2]
                elif style=="phase_portrait": img[py,px]=[abs(z)/50,0.3,1-abs(z)/50]
    if style=="density":
        density = np.clip(np.log1p(density)/np.log1p(density.max()),0,1) if density.max()>0 else density
        for y in range(H):
            for x in range(W):
                v = density[y,x]
                if cm=="gradient": img[y,x]=[v*0.8,v*0.3,v*0.5]
                elif cm=="iteration": img[y,x]=[v,0.3,1-v]
                elif cm=="velocity": img[y,x]=[v*0.5,v*0.8,v*0.3]
                elif cm=="divergence": img[y,x]=[v,0.5-v*0.5,0.5]
                elif pal: c = pal[int(v*(len(pal)-1))%len(pal)]; img[y,x]=[c[0]/255,c[1]/255,c[2]/255]
                else: img[y,x]=[v*0.8,v*0.3,v*0.5]
    if style=="trace":
        for px,py in trail:
            if 0<=px<W and 0<=py<H: img[py,px]=[0.8,0.4,0.1]
    if pal_name and pal_name in PALETTES: img = quantize_to_palette(img.clip(0,1),pal_name)
    capture_frame('62', img); save(img.clip(0,1), mn(62,"Chaotic Map"), out_dir)

@method(id="65", name="Waveform", category="math_art", tags=["audio","fast", "expanded"],
         params={"wave_type":{"description":"wave type","choices":["sine","sawtooth","square","triangle","pulse","am_modulated","fm_modulated","noise_floor","lissajous","spectrum_analyzer","harmonic_series","interference","phase_space","wavetable","granular"],"default":"sine"},
                 "freq1":{"description":"frequency 1","min":1,"max":50,"default":5},"freq2":{"description":"frequency 2","min":1,"max":50,"default":3},"freq3":{"description":"frequency 3","min":1,"max":50,"default":7},
                 "noise_level":{"description":"noise level","min":0.0,"max":1.0,"default":0.05},"amplitude_ratio":{"description":"amplitude ratio","min":0.1,"max":1.0,"default":0.8},
                 "layout":{"description":"layout","choices":["single","multi_track","stereo_pair","circular","ribbon_3d","equalizer","waterfall","polar_phase"],"default":"single"},
                 "style":{"description":"style","choices":["line","palette","gradient_fill","glow_trace","oscilloscope","spectrogram","filled_wave","particle_trace","neon_tube","heat_wave"],"default":"line"},
                 "palette":{"description":"PALETTES","default":""},"bg_style":{"description":"bg","choices":["dark","grid","gradient","scanline","light"],"default":"dark"},
                 "num_tracks":{"description":"tracks","min":2,"max":16,"default":4},"pulse_width":{"description":"pulse width","min":0.05,"max":0.95,"default":0.5},
                 "mod_freq":{"description":"modulation freq","min":1,"max":20,"default":2},"mod_depth":{"description":"modulation depth","min":0.0,"max":1.0,"default":0.5},
                 "decay_rate":{"description":"decay rate","min":0.5,"max":0.99,"default":0.9},"line_width":{"description":"line width","min":1,"max":5,"default":2},
                 "fill_alpha":{"description":"fill alpha","min":0.0,"max":1.0,"default":0.3},"num_bars":{"description":"bars","min":10,"max":200,"default":50},
                 "time":{"description":"time","min":0.0,"max":100.0,"default":0.0}})
def method_waveform(out_dir: Path, seed: int, params=None):
    ...
    t = float(params.get("time", 0.0))
    seed_all(seed + int(t * 100))
    wt = params.get("wave_type","sine"); f1 = params.get("freq1",5); f2 = params.get("freq2",3); f3 = params.get("freq3",7)
    nl = params.get("noise_level",0.05); ar = params.get("amplitude_ratio",0.8)
    layout = params.get("layout","single"); style = params.get("style","line")
    pal_name = params.get("palette",""); bg_style = params.get("bg_style","dark")
    nt = params.get("num_tracks",4); pw = params.get("pulse_width",0.5)
    mf = params.get("mod_freq",2); md = params.get("mod_depth",0.5)
    decay = params.get("decay_rate",0.9); lw = params.get("line_width",2)
    fa = params.get("fill_alpha",0.3); nb = params.get("num_bars",50)
    from ..core.utils import PALETTES, quantize_to_palette
    pal = PALETTES.get(pal_name,[])
    img = np.zeros((H,W,3),dtype=np.float32)
    if bg_style=="dark": img[:,:,:]=0.05
    ...
    def wave_val(x,t):
        ...
        if wt=="noise_floor": return random.uniform(-1,1)*nl+math.sin(x*f1*0.1+t)*0.3
    ...
    if layout=="single":
        pts = []
        for x in range(W):
            v = wave_val(x,t)+random.uniform(-nl,nl)
            y = int(H/2+v*ar*H/2)
            pts.append((x,y))
        if style=="line":
            ...
            for x in range(0,W,3):
                v = wave_val(x,t)+random.uniform(-nl,nl); y = int(H/2+v*ar*H/2)
        ...
        for tr in range(nt):
            f = f1+tr*2; pts = []
            for x in range(W):
                v = math.sin(x*f*0.1+t+tr*0.5)+random.uniform(-nl,nl)
        ...
        for ch in range(2):
            pts = []
            for x in range(W):
                v = math.sin(x*(f1+ch*2)*0.1+t)+random.uniform(-nl,nl)
        ...
        for a in range(360):
            th = a*math.pi/180; v = wave_val(a*W//360,t)+random.uniform(-nl,nl)
        ...
        for b in range(nb):
            v = abs(wave_val(b*W//nb,t))+random.uniform(-nl,nl)
        ...
        for tr in range(20):
            pts = []
            for x in range(W):
                v = math.sin(x*(f1+tr*0.5)*0.1+t+tr*0.3)+random.uniform(-nl,nl)
...
def method_fft_art(out_dir: Path, seed: int, params=None):
    ...
    t = float(params.get("time", 0.0))
    seed_all(seed + int(t * 100))
    ft = params.get("filter_type","ring"); src = params.get("source","random")
    cm = params.get("color_mode","gradient"); pal_name = params.get("palette","")
    n_rings = params.get("n_rings",5); r1c = params.get("ring1_center",60); r1s = params.get("ring1_sigma",15)
    r2c = params.get("ring2_center",120); r2s = params.get("ring2_sigma",20)
    st = params.get("spiral_turns",4); sa = params.get("star_arms",6); cks = params.get("checker_size",16)
    txt = params.get("text_content","FFT"); gf = params.get("gabor_freqs",4); go = params.get("gabor_orientations",4)
    fe = params.get("fractal_exponent",1.5); pss = params.get("phase_swap_source","perlin")
    kt = params.get("kernel_type","gaussian"); ks = params.get("kernel_size",7)
    prf = params.get("polar_radial_freq",4); paf = params.get("polar_angular_freq",6)
    from ..core.utils import PALETTES, quantize_to_palette
    pal = PALETTES.get(pal_name,[])
    # Generate source
    if src=="random": noise = np.random.randn(H,W)
    elif src=="perlin":
        yy,xx = np.ogrid[:H,:W]
    ...
    elif src=="color_noise": noise = np.random.randn(H,W,3)
    else: noise = np.random.randn(H,W)
    if noise.ndim==2: noise = np.stack([noise]*3,axis=-1)
    # FFT
    fft = np.fft.fft2(noise,axes=(0,1)); fft = np.fft.fftshift(fft,axes=(0,1))
    Hc,Wc = H//2,W//2; yy,xx = np.ogrid[:H,:W]
    r = np.sqrt((xx-Wc)**2+(yy-Hc)**2); theta = np.arctan2(yy-Hc,xx-Wc)
    # Build filter
    ...
    if ft=="ring":
        ...
        for i in range(n_rings): mask += np.exp(-(r-(i+1)*r2c/n_rings)**2/(2*r1s**2))
    elif ft=="spiral":
        ...
        for fi in range(gf):
            for oi in range(go):
                f = (fi+1)*10; o = oi*math.pi/go+t*0.5
                g = np.exp(-(r-f)**2/(2*20**2))*np.cos(theta-o)**2
                mask += g
        mask = mask/mask.max()
    elif ft=="fractal_noise":
        mask = r**(-fe); mask[H//2,W//2]=0; mask = mask/mask.max()
    elif ft=="polar_fft":
        ...
        for _ in range(20):
            ...
            sr = random.uniform(5,20); mask += np.exp(-((xx-cx)**2+(yy-cy)**2)/(2*sr**2))
        mask = mask/mask.max()
    ...
    for c in range(3):
        ...
        if ft=="phase_swap":
            ...
            elif pss=="random": pn = np.random.randn(H,W)
            else: pn = noise[:,:,0]
    ...
    if cm=="gradient": pass
        ...
        phase = np.angle(fft); img = np.stack([phase,phase*0.5,1-phase],axis=-1); img = norm(img)
    elif cm=="magnitude":
        mag = np.log1p(np.abs(fft)); img = norm(mag)
    elif cm=="multi_channel":
        pass
    elif cm=="channel_swap": img = img[:,:,[2,0,1]]
    elif cm=="rainbow":
        img = np.stack([img[:,:,0],img[:,:,1]*0.5,1-img[:,:,2]],axis=-1)
    elif cm=="heatmap":
        img = np.stack([img[:,:,0],img[:,:,1]*0.3,1-img[:,:,2]*0.5],axis=-1)
    if pal_name and pal_name in PALETTES: img = quantize_to_palette(img.clip(0,1),pal_name)
    capture_frame('48', img); save(img.clip(0,1), mn(48,"FFT Art"), out_dir)
...
def method_density_heatmap(out_dir: Path, seed: int, params=None):
    ...
    t = float(params.get("time", 0.0))
    seed_all(seed + int(t * 100))
    n_pts = params.get("points",5000); sigma = params.get("sigma",30)
    src = params.get("source","gaussian_cluster"); n_cl = params.get("n_clusters",4)
    style = params.get("style","colormap"); cmap = params.get("cmap","inferno")
    pal_name = params.get("palette",""); dual_cmap = params.get("dual_cmap","viridis")
    cl = params.get("contour_levels",8); sa = params.get("scatter_alpha",0.3)
    kt = params.get("kernel_type","gaussian"); la = params.get("light_angle",45)
    lalt = params.get("light_alt",30); rs = params.get("ridge_spacing",20)
    cshift = params.get("colormap_shift",0.0); ads = params.get("adaptive_sigma","no")
    ps = params.get("point_speed",0.0)
    from ..core.utils import PALETTES, quantize_to_palette
    pal = PALETTES.get(pal_name,[])
    # Generate points
    pts = []
    if src=="gaussian_cluster":
        cx,cy = W/2,H/2; pts = np.random.randn(n_pts,2)*np.array([sigma,sigma])+np.array([cx,cy])
    elif src=="grid_jitter":
        cols = int(math.sqrt(n_pts*W/H)); rows = n_pts//cols
        for r in range(rows):
            for c in range(cols): pts.append([(c+0.5)*W/cols+random.uniform(-sigma,sigma),(r+0.5)*H/rows+random.uniform(-sigma,sigma)])
        pts = np.array(pts)
    elif src=="multi_cluster":
        pts = []
        for _ in range(n_cl):
            cx,cy = random.uniform(W*0.2,W*0.8),random.uniform(H*0.2,H*0.8)
            s = random.uniform(sigma*0.3,sigma)
            pts.append(np.random.randn(n_pts//n_cl,2)*np.array([s,s])+np.array([cx,cy]))
        pts = np.vstack(pts)
    elif src=="spiral_path":
        pts = []
        for i in range(n_pts):
            th = i*0.1+t*ps; r = i*0.5
        ...
        edges = np.abs(noise)>0.3; cand = np.argwhere(edges)
        if len(cand)<n_pts: cand = np.array([[random.randint(0,H-1),random.randint(0,W-1)] for _ in range(n_pts)])
        idx = np.random.choice(len(cand),n_pts,replace=True)
        pts = cand[idx][:,::-1].astype(np.float32)
    else:
        pts = np.random.randn(n_pts,2)*np.array([sigma,sigma])+np.array([W/2,H/2])
    # Density
    density = np.zeros((H,W),dtype=np.float32)
    for x,y in pts:
        ix,iy = int(x),int(y)
        if 0<=ix<W and 0<=iy<H: density[iy,ix]+=1
    if kt=="gaussian": density = cv2.GaussianBlur(density,(0,0),sigmaX=sigma,sigmaY=sigma)
    elif kt=="exponential": density = np.exp(cv2.GaussianBlur(density,(0,0),sigmaX=sigma,sigmaY=sigma)*3)
    elif kt=="epanechnikov":
    ...
    img = np.zeros((H,W,3),dtype=np.float32)
    if style=="colormap":
        img[:,:,0] = density*0.8; img[:,:,1] = density*0.3; img[:,:,2] = density*0.5
    elif style=="contour_overlay":
        img[:,:,0] = density*0.8; img[:,:,1] = density*0.3; img[:,:,2] = density*0.5
        for level in np.linspace(0.1,0.9,cl):
        ...
        la_rad = la*math.pi/180; lalt_rad = lalt*math.pi/180
        ...
        for y in range(0,H,rs):
            v = density[y,:]; img[y:y+rs//2,:,0] = v*0.8; img[y:y+rs//2,:,1] = v*0.3; img[y:y+rs//2,:,2] = v*0.5
    elif style=="stippled":
        for y in range(H):
            for x in range(W):
                if random.random()<density[y,x]: img[y,x] = [0.8,0.6,0.1]
        ...
        img[:,:,0] = d1*0.8; img[:,:,1] = d2*0.3; img[:,:,2] = d2*0.5
    elif style=="edge_map":
        img[:,:,0] = density*0.8; img[:,:,1] = density*0.3; img[:,:,2] = density*0.5
        edges = cv2.Canny((density*255).astype(np.uint8),50,150)
        img[edges>0] = [1,1,1]
    if pal_name and pal_name in PALETTES: img = quantize_to_palette(img.clip(0,1),pal_name)
    capture_frame('43', img); save(img.clip(0,1), mn(43,"Density Heatmap"), out_dir)
