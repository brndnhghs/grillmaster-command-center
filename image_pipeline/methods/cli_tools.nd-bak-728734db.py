...
def method_pyfiglet(out_dir: Path, seed: int, params=None):
    ...
    try:
        ...
        save(fallback, mn(24, 'pyfiglet'), out_dir)
        print(f'[method_24] ERROR: {exc}')
        return fallback
...
def method_boxes(out_dir: Path, seed: int, params=None):
    ...
    try:
        ...
        fallback = np.full((H, W, 3), 128, dtype=np.uint8)
        save(fallback, mn(25, 'boxes'), out_dir)
...
def method_cowsay(out_dir: Path, seed: int, params=None):
    ...
    try:
        ...
        fallback = np.full((H, W, 3), 128, dtype=np.uint8)
        save(fallback, mn(26, 'cowsay'), out_dir)
        print(f'[method_26] ERROR: {exc}')
        return fallback
...
def method_qrencode(out_dir: Path, seed: int, params=None):
    ...
    try:
        ...
        fallback = np.full((H, W, 3), 128, dtype=np.uint8)
        save(fallback, mn(27, 'qrencode'), out_dir)
        print(f'[method_27] ERROR: {exc}')
        return fallback
...
@method(id="44", name="img2txt", category="cli_tools", tags=["text", "caca", "expanded"],
        params={
            ...
            "font_size": {"description": "PIL font size for rendering", "min": 6, "max": 48, "default": 10},"anim_mode": {"description": "animation mode", "choices": ["none", "circle_morph", "char_cycle"], "default": "none"},
...
def method_img2txt(out_dir: Path, seed: int, params=None):
    ...
    try:
        ...
        print(f"  ✗ img2txt: source save failed: {e} — writing source image as fallback")
        fallback = img.resize((W, H), Image.LANCZOS).convert("RGB")
        capture_frame("44", np.array(fallback, dtype=np.float32) / 255.0)
        save(fallback, mn(44, "img2txt"), out_dir)
        return
    ascii_text = ""
    try:
...
@method(id="45", name="Graphviz", category="cli_tools", tags=["graph", "expanded"],
        params={
            ...
            "dpi": {"description": "output DPI", "min": 36, "max": 300, "default": 72},"anim_mode": {"description": "animation mode", "choices": ["none", "edge_morph", "color_cycle",
...
@method(id="46", name="ImageMagick Plasma", category="cli_tools", tags=["imagemagick", "expanded"],
        params={
            ...
            "min_bytes": {"description": "minimum output file size to accept", "min": 100, "max": 100000, "default": 1000},"anim_mode": {"description": "animation mode", "choices": ["none", "plasma_pulse", "blur_cycle", "tile_cycle", "seed_morph", "oil_shock"], "default": "none"},
            "anim_speed": {"description": "animation speed multiplier", "min": 0.1, "max": 5.0, "default": 1.0},
        })
...
@method(id="47", name="Chafa", category="cli_tools", tags=["text", "caca", "expanded"],
        params={
            ...
            "font_size": {"description": "PIL font size for rendering", "min": 6, "max": 48, "default": 10},"anim_mode": {"description": "animation mode", "choices": ["none", "shape_morph", "color_cycle"], "default": "none"},
            "anim_speed": {"description": "animation speed multiplier", "min": 0.1, "max": 5.0, "default": 1.0},
        })
