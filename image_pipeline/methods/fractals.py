...
@method(id="66", name="Julia Set", category="fractals", tags=["classic", "fast", "expanded", "animation"],
    ...
    "animation_mode": {"description": "animation: none, morph, zoom, color_cycle, param_morph, flame", "default": "none"},
...
def method_julia_set(out_dir: Path, seed: int, params=None):
    ...
    for i in range(max_iter):
        ...
        if anim_mode != "none" and i % frame_interval == 0 and i > 0:
            d_preview = div.astype(np.float32) / max_iter
            cap = np.stack([
                np.sin(d_preview * c_speed + c_off) * 0.5 + 0.5,
                np.sin(d_preview * c_speed * 0.75 + 2 + c_off) * 0.5 + 0.5,
                np.sin(d_preview * c_speed * 0.5 + 4 + c_off) * 0.5 + 0.5,
            ], axis=-1)
    ...
    if color_mode == "sine":
        ...
        r = np.clip(d * 3.0 + c_off * 0.3, 0, 1)
        g = np.clip(d * 2.0 - 0.3 + c_off * 0.2, 0, 1)
        b = np.clip(d * 1.5 - 0.5, 0, 1)
        result = np.stack([r, g, b], axis=-1)

    elif color_mode == "smooth_gradient":
        frac = div_arr / max_iter
        t_grad = (frac * c_speed + c_off) % 1.0
        ...
        r = np.sin(angle_map * np.pi * 6 + c_off) * 0.5 + 0.5
        g = np.sin(angle_map * np.pi * 6 + 2.1 + c_off) * 0.5 + 0.5
        b = np.sin(angle_map * np.pi * 6 + 4.2 + c_off) * 0.5 + 0.5
        result = np.stack([r, g, b], axis=-1)
    ...
    if np.any(interior):
        if interior_color == "black":
            ...
            r = np.sin(it_norm * 2 + c_off) * 0.2 + 0.1
            g = np.sin(it_norm * 2 + 2 + c_off) * 0.2 + 0.1
            b = np.sin(it_norm * 2 + 4 + c_off) * 0.2 + 0.1
    ...
    # ── Color cycle animation ──
    if anim_mode == "color_cycle":
        hue_shift = (math.sin(t * 0.5) * 0.5 + 0.5) * 0.5
        result = np.roll(result * 255, int(hue_shift * 255), axis=-1) / 255.0
...
@method(id="72", name="Pythagorean Tree", category="fractals", tags=["recursive", "colorful", "expanded", "animation"],
    ...
    "animation_mode": {"description": "animation: none, grow, sway, wind, color_cycle, pulse, breath", "default": "none"},
    "anim_speed": {"description": "animation speed", "min": 0.1, "max": 3.0, "default": 1.0},
...
def method_pythagorean_tree(out_dir: Path, seed: int, params=None):
    ...
    def get_color(d, max_d, x, y):
        ...
        if color_mode == "depth_gradient":
            ...
            hue = (frac * c_speed + c_off) % 1.0
            r = int((np.sin(hue * np.pi * 6) * 0.5 + 0.5) * 255)
            g = int((np.sin(hue * np.pi * 6 + 2.1) * 0.5 + 0.5) * 255)
            b = int((np.sin(hue * np.pi * 6 + 4.2) * 0.5 + 0.5) * 255)
            return (r, g, b)
        elif color_mode == "neon":
            r = int((np.sin(frac * np.pi * 4 + c_off) * 0.5 + 0.5) * 200 + 55)
            g = int((np.sin(frac * np.pi * 4 + 2.1 + c_off) * 0.5 + 0.5) * 200 + 55)
            b = int((np.sin(frac * np.pi * 4 + 4.2 + c_off) * 0.5 + 0.5) * 200 + 55)
            return (r, g, b)
            ...
            hue = ((x / W + y / H) * 0.5 + c_off / 6.28) % 1.0
    ...
    def branch(x, y, length, angle, d):
        ...
        # Angle variation
        av = random.uniform(-ang_var, ang_var) if ang_var > 0 else 0
    ...
    if anim_mode == "breath":
        arr = np.array(img, dtype=np.float32)
        breath = 0.5 + 0.5 * math.sin(t * 0.8 * anim_speed)
