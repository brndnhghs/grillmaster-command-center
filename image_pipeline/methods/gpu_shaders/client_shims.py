"""
Client-GPU preview shims — pure data, no logic.

CLIENT_GPU_SHIMS routes existing CPU nodes' LIVE previews to closed-form GLSL
twins (core/shaders.py); CLIENT_GPU_SIMS routes Arch-A simulation nodes to
WebGL2 ping-pong sim twins. GPU_PREVIEW_DROP_ALLOW + is_param_justified_drop
enforce the no-silent-param-drops coverage contract.

All of it is merged into GPU_SHADER_NODE_MAP by _shared.py and served via
/api/shader-sources; the CPU numpy fns stay authoritative for export
(two-tier precision, GPU-First guardrail).
"""
...
GPU_PREVIEW_DROP_ALLOW: dict[str, dict[str, str]] = {
    ...
    # ── Post-renumbering (85512e2) numeric sliders with no twin uniform ──
    # Documented so the no-silent-drops guard passes; each is a CPU-domain or
    # non-per-pixel knob the closed-form twin intentionally does not model.
    "386": {
        "tint": "hue scalar; the god_rays_gpu twin hardcodes a white sun (avoids a vec3 uniform); CPU export authoritative for tint",
        "num_samples": "constant loop bound; the twin hardcodes 64 samples (GLSL constant); CPU export authoritative",
    },
    "370": {
        "noise_amp": "non-local source-noise knob (not per-pixel); rolling_shutter_gpu warps the wired image; CPU export authoritative",
        "blur_sigma": "non-local blur sigma (not per-pixel); rolling_shutter_gpu has no blur pass; CPU export authoritative",
    },
    "453": {
        "albedo_r": "twin hardcodes a pearl-ish studio matcap (albedo is baked in); CPU export honours the exact albedo choice",
        "albedo_g": "twin hardcodes a pearl-ish studio matcap (albedo is baked in); CPU export honours the exact albedo choice",
        "albedo_b": "twin hardcodes a pearl-ish studio matcap (albedo is baked in); CPU export honours the exact albedo choice",
    },
    "395": {
        "ambient": "fixed ambient light in cel_shading_gpu; CPU export honours the ambient slider",
    },
}
